...
import os
import util
from External import Figure_Plot


class GMM_base_AE():
    def __init__(self, flag_cuda, num_cluster, learning_rate, dim, data_loader, input_dim,\
        batch_size, c_dim=30, save_path='.'):
        ...
        if flag_cuda:
            self.EM_2head_AE = EM_base_AE(num_classes=self.num_cluster, dim=self.dim, img_dim=input_dim, c_dim=c_dim).cuda()
        else:
            self.EM_2head_AE = EM_base_AE(num_classes=self.num_cluster, dim=self.dim, img_dim=input_dim, c_dim=c_dim)
        self.loss_func1 = torch.nn.MSELoss()
        self.loss = torch.nn.MSELoss()
        self.optimizer = torch.optim.Adam(self.EM_2head_AE.parameters(), lr=self.lr) #, weight_decay=5e-6)
    ...
    def train(self, epoch):
        ...
        for i in range(train_data.shape[0]//batch_size):
            ...
            c_label = train_targets[i*batch_size:(i+1)*batch_size, 1]
            c_label = torch.tensor(c_label, dtype=torch.long)
            c_onehot = util.one_hot(c_label, NUM_LABELS=self.c_dim)
            if self.flag_cuda:
                data = batch_x.cuda()
                y_true = y_true.cuda()
                c_onehot = c_onehot.cuda()
            else:
                data = batch_x
            self.optimizer.zero_grad()
            z, quantized, gamma, recon_x, recon_center, mu, log_var = self.EM_2head_AE(data, c_onehot)
            ...
            loss5 = util.kl_conditional_and_marg(mu, log_var, self.dim, gauss_ent=False)
            loss = loss1 + ep1 * loss2 + ep2 * loss3 + ep3 * loss4 + ep4 * loss5
            loss.backward()
            self.optimizer.step()
            if self.use_tensorboard:
                self.tensorboard.add_scalar('loss/recon loss', loss1, i+epoch*(train_data.shape[0]//batch_size))
                self.tensorboard.add_scalar('loss/kld loss', loss4, i+epoch*(train_data.shape[0]//batch_size))
                self.tensorboard.add_scalar('loss/kl_qzx_qz loss', loss5, i+epoch*(train_data.shape[0]//batch_size))
            ...
            if batch_idx % 100 == 0:
                print('Train Epoch: {} ({:.0f}%) \tLoss1:{:.6f}\tLoss2:{:.6f}\tLoss4:{:.6f}\tLoss5:{:.6f}\tGamma:{:.6f}'\
                    .format(epoch, 100. * batch_idx / (train_data.shape[0]//batch_size), loss1.data, loss2.data, \
                        loss4.data, loss5.data, max_gamma.mean()))
    ...
    def eval(self, epoch):
        ...
        for i in range(n_batch):
            ...
            c_label = train_targets[i*batch_size:(i+1)*batch_size, 1] # domain label
            batch_x = torch.tensor(batch_x, dtype=torch.float)
            y_true = torch.tensor(y_true, dtype=torch.long)
            c_label = torch.tensor(c_label, dtype=torch.long)
            c_onehot = util.one_hot(c_label, NUM_LABELS=self.c_dim)
            if self.flag_cuda:
                ...
                data = batch_x
            z, quantized, gamma, recon_x, recon_center, _, _ = self.EM_2head_AE(data, c_onehot)
            max_gamma, y_pred = gamma.max(dim=1)
            if self.flag_cuda:
                y_true = y_true.cpu().data.numpy()
                true_label.append(y_true)
    ...
    def process(self, Iter=1001):
        for epoch in range(Iter):
            if epoch % 50 == 0 or epoch == 1:
                ...
                file_name = self.save_path+'embedding{}.png'.format(epoch)
    ...
    def get_embedding(self):
        ...
        for i in range(n_batch):
            batch_x = train_data[i*batch_size:(i+1)*batch_size]
            y_true = train_targets[i*batch_size:(i+1)*batch_size, 0] # object label
            domain_labels.append(train_targets[i*batch_size:(i+1)*batch_size, 1])
            batch_x = torch.tensor(batch_x, dtype=torch.float)
        ...
        if self.flag_cuda:
            ...
            centroid = self.EM_2head_AE.Clustering.centroids.weight.data.cpu().numpy()
        else:
